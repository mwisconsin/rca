...
<table class="center_cell_data" border="1" width="870px">
   <caption><span class="table_caption">Rides by Category</span></caption>
   <tr>
      <td></td>
      <th>Completed</th>
      <th>Scheduled</th>
...
<br />
<table class="center_cell_data" border="1" width="850px" id="ride_collapse">
   <caption><span class="table_caption">Rides by Category - Data</span></caption>
   <tr>
...
//show requested information within date range
      ...
      $sql = "SELECT link.FranchiseID, 'SCHEDULED' AS LinkStatus, DesiredArrivalTime, RiderUserID, NumberOfRiders, 
                       ToDestinationID, QuotedCents, Distance, AssignedDriverUserID AS DriverUserID,
                       users.PersonNameID, Title, FirstName, MiddleInitial, LastName, Suffix,
                       destination.Name, CustomTransitionType
                ...
                UNION 
                SELECT link_history.FranchiseID, LinkStatus, DesiredArrivalTime, RiderUserID, NumberOfRiders, 
                       ToDestinationID, QuotedCents, Distance, DriverUserID, 
                       users.PersonNameID, Title, FirstName, MiddleInitial, LastName, Suffix, 
                       destination.Name, CustomTransitionType
      ...
      while ($row = mysql_fetch_array($result)) {
         ...
         /* C4 = Rider Name */
         echo "<td>{$row['NumberOfRiders']}</td>"; /* C5 = number of riders */
         echo "<td>{$row['Name']}</td>"; /* C6 = destination */
         echo "<td>";
         printf("$%d.%02.2d", $row['QuotedCents'] / 100, $row['QuotedCents'] % 100);
         echo "</td>";    /* C7 = Fee Paid for Link */
         echo "<td>{$row['Distance']}</td>";  /* C8 = Mileage */
         $driver = $row['DriverUserID'] ? get_displayable_person_name_string( get_user_person_name( $row['DriverUserID'] )) : 'Not Set';
         echo "<td>" . $driver . "</td>";     /* C9 = Driver */
         
         $CTT = $row['CustomTransitionType'];
         //$ctt = get_CustomTransitionType_string( $row['CustomTransitionType'] );
            
         echo "<td>" . $CTT . "</td>";      // C10 =  Custom Transition Type for this event
         //echo "<td>" . $ctt . "</td>";     // C10 =  Custom Transition Type for this event
...
<?php 
   ...
   // $new_riders = get_number_of_new_riders($franchise, $dates['From'], $dates['To']);
   //------------------------------------------------
   // This is a modified copy of orig fn
   /////$debug_new_riders = get_riders_having_first_ride($franchise, $dates['From'], $dates['To']);
...
<button type="button" id="rider" style="float: right;">Show
   Data</button>
<table class="center_cell_data" border="1" width="800px">
   <caption><span class="table_caption">Rider Summary</span></caption>
   <tr>
      <td></td>
      <th>Completed</th>
      <th>Scheduled</th>
...
<br />
<table class="center_cell_data" border="1" width="850px" id="rider_collapse"> <!-- Tim Change to 850px from 750 -->
   <caption><span class="table_caption">Rider - Data</span></caption>
   <tr>
...
<button type="button" id="driver" style="float: right;">Show
   Data</button>
<table class="center_cell_data" border="1" width="800px">
   <caption><span class="table_caption">Driver Summary</span></caption>
   <tr>
      <td></td>
      <th>Completed</th>
      <th>Scheduled</th>
...
<br />
<table class="center_cell_data" border="1" width="850px" id="driver_collapse">
   <caption><span class="table_caption">Driver - Data</span></caption>
   <tr>
      <th>UserID</th>
      <th>Name</th>
      <th>New</th> 
...
<br />
<table class="center_cell_data" border="1" width="800px">
   <caption><span class="table_caption">Rider / Driver Ratios</span></caption>
   <tr>
      <td></td>
      <th>Completed</th>
      <th>Scheduled</th>
...
<br />
<table class="center_cell_data" border="1" width="800px">
   <caption><span class="table_caption">Link Ratios</span></caption>
   <tr>
      <td></td>
      <th>Completed</th>
      <th>Scheduled</th>
...
?>
<table class="rightalign_cell_data" border="1" width="870px">
   <caption><span class="table_caption">Revenue by Category</span></caption>
   <tr>
      <td></td>
      <th>Completed</th>
      <th>Scheduled</th>
...
<br />
<table class="center_cell_data" width="400px" border="1">
   <caption><span class="table_caption">Geographic Summary (by Zip)</span></caption>
   <tr>
      <th>Zip</th>
      <th>City</th>
      <th># Departs</th>
...
// ADMIN ONLY (Full or Club)CAN view report information
   ...
   if(current_user_has_role(1, "FullAdmin") && current_user_has_role($Club, "Franchisee")){
      header("Location: home.php");
      die();   
   }
...
//DATE RANGE being reviewed, start with today  *** CHANGE TO CURRENT MONTH 
   ...
   $results = get_rider_report_numbers($Club, $dates['From'],$dates['To']);
   $months = array('January','February','March','April','May','June','July','August','September','October','November','December');
    include_once 'include/header.php';
?>
<center><h2>Ride Reports</h2></center>
...
/*THIS MAY BE WHERE THE CUSTOM TRANSITION TYPE NEEDS TO BE FILTERED <> DRIVER WHEN LOOKING AT 
 LINK_HISTORY, FORMULA IN LINK.PHP  */
   ...
   $links = get_number_of_links($Club, $dates['From'],$dates['To']);
   $distance = get_distance_of_links($Club, $dates['From'],$dates['To']);
   $revenue = get_revenue_of_links($Club, $dates['From'],$dates['To']);

   $transition = get_transition_data($Club, $dates['From'], $dates['To']);
   //print_r($transition);
?>
...
//show requested information within date range
      ...
      $safe_Club = mysql_real_escape_string($Club);
      $sql = "SELECT link.ClubID, 'SCHEDULED' AS LinkStatus, DesiredArrivalTime, RiderUserID, ToDestinationID, QuotedCents, 
                       Distance, AssignedDriverUserID AS DriverUserID,
                       users.PersonNameID, Title, FirstName, MiddleInitial, LastName, Suffix,
                       destination.Name, CustomTransitionType
                       FROM link LEFT JOIN (users NATURAL JOIN person_name) ON RiderUserID = UserID 
                                 LEFT JOIN destination ON ToDestinationID = destination.DestinationID
                       WHERE $date_clause AND link.ClubID = $safe_Club 
                UNION 
                SELECT link_history.ClubID, LinkStatus, DesiredArrivalTime, RiderUserID, ToDestinationID, QuotedCents, Distance, DriverUserID,
                       users.PersonNameID, Title, FirstName, MiddleInitial, LastName, Suffix,
                       destination.Name, CustomTransitionType
                       FROM link_history LEFT JOIN (users NATURAL JOIN person_name) ON RiderUserID = UserID 
                                         LEFT JOIN destination ON ToDestinationID = destination.DestinationID
                       WHERE $date_clause AND link_history.ClubID = $safe_Club 
                       ORDER BY DesiredArrivalTime";
      $result = mysql_query($sql);
        if (!$result) {
            rc_log_db_error(PEAR_LOG_ERR, mysql_error(), 
...
<?php 
   $riders = get_number_of_paying_riders($Club, $dates['From'],$dates['To']); 

   //THIS FORMULA WORKS FOR NEW RIDERS, FORMULA FOR HIDDEN TABLE IS WRONG
   
   $new_riders = get_number_of_new_riders($Club, $dates['From'], $dates['To']);
?>
<br />
<button type="button" id="rider" style="float: right;">Show
   Data</button>
   ...
   <?php
        $sql = "SELECT RiderUserID, COUNT(*) AS Links, SUM(Distance) AS Distance, 
                                    ...
                                    (SELECT MIN(DesiredArrivalTime) FROM link_history
                                            WHERE RiderUserID = t1.RiderUserID) AS LinkHistMinArrivalTime,
                                    users.PersonNameID, Title, FirstName, MiddleInitial, LastName, Suffix
                FROM ( SELECT RiderUserID, LinkStatus, DesiredArrivalTime, Distance, QuotedCents, ClubID FROM link
                       UNION
                       SELECT RiderUserID, LinkStatus, DesiredArrivalTime, Distance, QuotedCents, ClubID FROM link_history ) t1
                     LEFT JOIN (users NATURAL JOIN person_name) ON RiderUserID = UserID 
                WHERE $date_clause AND ClubID = $safe_Club AND
                      LinkStatus IN ('COMPLETE', 'CANCELEDLATE', 'UNKNOWN')
                GROUP BY RiderUserID";
...
<?php
   $drivers = get_number_of_working_drivers($Club, $dates['From'], $dates['To']);
   $new_drivers = get_number_of_new_drivers($Club, $dates['From'], $dates['To']);
   ...
   <tr>
      ...
      <?php
         $sql = "SELECT *, IF( (SELECT DesiredArrivalTime 
                                      ...
                                      WHERE DriverUserID = t1.DriverUserID LIMIT 1) BETWEEN 
                                            '{$dates['To']}' AND '{$dates['From']}', 'Yes','No') AS New 
                    FROM (SELECT ClubID, AssignedDriverUserID DriverUserID, DesiredArrivalTime 
                          FROM link 
                          UNION 
                          SELECT ClubID, DriverUserID, DesiredArrivalTime 
                          FROM link_history) t1 
                    WHERE DriverUserID IS NOT NULL AND $date_clause AND ClubID = $safe_Club
                    GROUP BY DriverUserID";
...
<table width="400px" border="1">
   ...
   <?php    
        $sql = "SELECT ZIP5, City, COUNT(ZIP5) AS Depart
                FROM (SELECT LinkID, FromDestinationID FROM link
                          WHERE LinkStatus IN ('COMPLETE', 'CANCELEDLATE', 'UNKNOWN') AND $date_clause AND ClubID = $safe_Club
                      UNION
                      SELECT LinkID, FromDestinationID FROM link_history
                          WHERE LinkStatus IN ('COMPLETE', 'CANCELEDLATE', 'UNKNOWN') AND $date_clause AND ClubID = $safe_Club
                      ) all_link, destination NATURAL JOIN address
