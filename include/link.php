...
function calculate_riders_incomplete_ride_costs( $rider_user_id ){
	$safe_rider_user_id = mysql_real_escape_string($rider_user_id);
...
function get_links_rider_names($links){
	foreach($links as $id => $link)
		$links[$id] = mysql_real_escape_string($link);
	$sql = "SELECT UserID, FirstName, MiddleInitial, LastName FROM (link LEFT JOIN users ON link.RiderUserID = users.UserID) LEFT JOIN person_name ON users.PersonNameID = person_name.PersonNameID WHERE LinkID ='" . implode("' || LinkID = '", $links) . "' GROUP BY UserID";
	$result = mysql_query($sql) or die($sql);
