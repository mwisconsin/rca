<?php
	...
	if(isset($_GET['type']) && $_GET['type'] == 'applicants' || $_GET['type'] == 'all'){
        ...
        $users = get_all_active_rider_user_info($franchise_id, (isset($_GET['sortby']) && $_GET['sortby'] == 'LastName') );
		$riders = array();
		foreach($users as $user){
			$riders[] = $user['UserID'];
		}
				...
				//while($row = mysql_fetch_array($result)){
					...
					<tr id="<?php echo $user_row['UserID']; ?>">
                        ...
                        <td class="black_over_white_center" ><?php 
                            if (isset($travel_times[$user_row['UserID']])) {
			...
			<?php
                ...
                foreach ($carefacilities as $carefacility_row) {
					...
					<tr id="<?php echo $carefacility_row['CareFacilityID']; ?>">
						<td style="text-align:center;"><a href="care_facility.php?id=<?php 
                                echo $carefacility_row['CareFacilityID']; ?>"><?php 
						...
						<td id="LastRideDate"><a href="care_facility_rides.php?fid=<?php echo $carefacility_row['CareFacilityID'];
																				echo "\">";
                                        echo format_date($carefacility_row['LastRide'], "n/j/y"); ?></a></td>
                        <td><?php 
                            if ($carefacility_row['NextRide']) {
                                echo "<a href=\"care_facility_invoice.php?id=".$carefacility_row['CareFacilityID']."\">".format_date( $carefacility_row['NextRide'], "n/j/y")."</a>";
                            } else {
                                echo ' - ';
                            }
                        ?></td>
                        <td style="text-align:right;"><a href="cf_ledger.php?id=<?php 
                        	echo $carefacility_row['CareFacilityID']."\">";
                        	printf("$%d.%02.2d", $carefacility_row['Balance']/100, $carefacility_row['Balance']% 100); ?></a></td>
                        <td  style="text-align:right;"<?php 
                                ...
                                echo "<a href=\"edit_care_facility.php?action=edit_balance&id=".$carefacility_row['CareFacilityID']."\">";
                                echo format_dollars($carefacility_row['AvailableBalance'] ); ?></a></td>
                        <td style="text-align:right;"><?php echo format_dollars($carefacility_row['RechargeThreshold']); ?></td>
                        <td nowrap><?php echo $carefacility_row['PhoneNumber'];  ?></td>
...
<!-- Active rider code begin -->
        ...
        $users = get_all_inactive_rider_user_info($franchise_id, (isset($_GET['sortby']) && $_GET['sortby'] == 'LastName') );
		$riders = array();
		foreach($users as $user){
			$riders[] = $user['UserID'];
		}
		...
		?>
		<h2 style="text-align:center;">Inactive Riders</h2>
        <a href="<?php if(!isset($_GET['sortby']) || $_GET['sortby'] == "LastName") echo $_SERVER['PHP_SELF'] . "?type=" . $_GET['type'] . "&sortby=FirstName";  else echo $_SERVER['PHP_SELF'] . "?type=" . $_GET['type'] . "&sortby=LastName"; ?>">
        <input type="button" value="<?php if(!isset($_GET['sortby']) || $_GET['sortby'] == "LastName") echo "Sort By First Name";  else echo "Sort By Last Name"; ?>" />
        </a>
		<table id="test2" class=" add_titles" width="1000px" border="1px" cellspacing="0px">
		...
		<table width="100%" border="1px" cellspacing="0px">
			...
			<?php
				if($supporting_friends) foreach($supporting_friends as $row){
                    ...
                    $phone = isset($phone_numbers[$row['UserID']]) ? $phone_numbers[$row['UserID']] : "";
					...
					<tr>
                        ...
                        <td><?php 
                        	if (isset($row['RiderUserID']))
						    {                            
						    	if (isset($travel_times[$row['RiderUserID']])) {
                        ...
                        <?php 
						 if (isset($row['RiderUserID']))
						    {
                                if ($available <= $row['RechargeThreshold'] && !isset($row['CareFacility'])) {
                                    echo " class=\"Table_Warning_Cell\">"; 
                                } else {  
                                    echo '>';
                                }
		...
		<table  id="sortabletable" class="sortable add_titles" width="1000px" border="1px" cellspacing="0px">
			...
			<?php
				...
				foreach($drivers as $row){
					...
					$pref = isset($alloc[$row['UserID']]) ? $alloc[$row['UserID']] : array();
					$Chighlight = "";
					if(@$pref['AllocationType'] == "REIMBURSEMENT"){
					   $pref = "DRIVER";
					} else if(@$pref["AllocationType"] == "ADDTORIDERBALANCE") {
						$pref = "Future Ride Account";
            ...
            ?>
                        ...
                        <td nowrap><?php echo $phone; ?></td>
                        <td id="DriverPictureWaiver" class="editable_date" style="text-align:center;"><?php echo isset($user_row['DriverPictureWaiver']) ? format_date($user_row['DriverPictureWaiver'], "n/j/y") : ""; ?></td>
					</tr>
					<?
				}
			?>
