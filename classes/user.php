...
class User{
    ...
    function __construct($user_id = 0, $working_as = 0, $should_redirect = true){
		...
		if(isset($_SESSION) && isset($_SESSION['UserFranchiseID'])){
		...
		if ($user_id > 0){
			...
			$sql = 
			"SELECT u.DefaultFranchiseID, n.Title, n.FirstName, n.MiddleInitial, n.LastName, n.Suffix, a.Address1, a.Address2, a.City, a.State, a.Zip5, a.Zip4,
			 e.EmailAddress, p.PhoneNumber, p.PhoneType, cu.CareFacilityID
			 FROM users u
			 LEFT JOIN person_name n ON n.PersonNameID = u.PersonNameID
			 LEFT JOIN user_address ua ON ua.UserID = u.UserID
			 LEFT JOIN address a ON a.AddressID = ua.AddressID
			 LEFT JOIN email e ON u.EmailID = e.EmailID 
			 LEFT JOIN user_phone up ON up.UserID = u.UserID
			 LEFT JOIN care_facility_user cu ON cu.UserID = u.UserID AND DATE(cu.ConnectionDate) <= DATE(NOW()) AND DATE(cu.DisconnectDate) > DATE(NOW()) 
			 NATURAL LEFT JOIN phone p 
			 WHERE u.UserID = $safe_user_id && AddressType = 'Physical' 
			 ORDER BY IsPrimary, PhoneType ";
			 
			 //echo 'user id = ' . $safe_user_id;
			 //echo $sql . '<br>';
			...
			if($result){
				while($row = mysql_fetch_array($result)){
					...
					$this->_CareFacilityID = $row['CareFacilityID'];
					if ($this->_club_id == 0){
...
// now if working as is -1 then see if they are working as someone else
	...
	function get_header_string(){
		//echo '$this->_UserID = ' . $this->_UserID . '<br>';
		if ($this->_UserID <= 0){
			return '';
		}
		$output_string = "";
