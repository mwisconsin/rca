    ...
    <form method="post" action="/login-action.php">
        <table style="text-align:center; margin:auto;">
            <tr>
                ...
                <td><input name="UserName" id="txt_user_name" type="text" value="<?php echo isset($_COOKIE['UserName']) ? $_COOKIE['UserName'] : ""; ?>" style="width:200px;" /></td>
            </tr>
            <tr>
                <td></td>
...
<?php
	...
	while($row = mysql_fetch_assoc($result)){
		...
		if(is_logged_in() && current_user_has_role($franchise, 'FullAdmin')){
			echo ' - <a href="/frontpage.php?action=delete&id=' . $row['FrontPageTextID'] . '">Delete</a>';
			echo ' <a href="/frontpage.php?action=edit&id=' . $row['FrontPageTextID'] . '">Edit</a>';
		}
			
		echo '</p>';	
	}
	if(is_logged_in() && current_user_has_role(1, 'FullAdmin'))
		echo '<a href="/frontpage.php?action=add">Add New Paragraph</a>';
	echo '</div>';
