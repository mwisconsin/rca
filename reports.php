...
//DATE RANGE being reviewed, start with today  *** CHANGE TO CURRENT MONTH 
   ...
   $dates = array('From' => date("Y-m-d") . " 00:00:00",
                  'To' => date("Y-m-d") . " 23:59:59" );
   if(!isset($_REQUEST['by'])) $_REQUEST['by'] = 'range';
   
   if(isset($_POST['FromMonth'])){
      $to = mysql_real_escape_string(str_pad($_POST['ToYear'],2,'0',STR_PAD_LEFT)) . '-' . mysql_real_escape_string(str_pad($_POST['ToMonth'],2,'0',STR_PAD_LEFT)) . '-' . mysql_real_escape_string(str_pad($_POST['ToDay'],2,'0',STR_PAD_LEFT));
      $from = mysql_real_escape_string(str_pad($_POST['FromYear'],2,'0',STR_PAD_LEFT)) . '-' . mysql_real_escape_string(str_pad($_POST['FromMonth'],2,'0',STR_PAD_LEFT)) . '-' . mysql_real_escape_string(str_pad($_POST['FromDay'],2,'0',STR_PAD_LEFT));
      
      $dates = array('From' => $from . " 00:00:00",
                  'To' => $to . " 23:59:59" );
      
   } else if(isset($_REQUEST['Year']) && !isset($_REQUEST['Month']) && !isset($_REQUEST['Day'])){
      $year = mysql_real_escape_string($_REQUEST['Year']);
      
      $dates = array('From' => "$year-01-01 00:00:00",
      ...
      $dates = array('From' => "$year-$month-01 00:00:00",
...
/*THIS MAY BE WHERE THE CUSTOM TRANSITION TYPE NEEDS TO BE FILTERED <> DRIVER WHEN LOOKING AT 
 LINK_HISTORY, FORMULA IN LINK.PHP  */
   ...
   #echo $dates['From'].','.$dates['To']."<BR>";
   $link_percentage = "0%";
   $sql = "select LinkID
  						FROM `link_history` 
   						WHERE `DesiredArrivalTime` between '{$dates['From']}' and '{$dates['To']}' 
   							and `LinkStatus` in ('COMPLETE','CANCELEDLATE')
   							and (CustomTransitionType is NULL or CustomTransitionType = 'RIDER')
   							and FranchiseID = $franchise";
   $r = mysql_query($sql); /* avoid DIV/0 */
   if(mysql_num_rows($r) > 0) {
	   $sql = "SELECT sum(case when time(`DesiredArrivalTime`) >= time(`ReportedArrivalTime`) then 1 else 0 end) / count(LinkID) 
	   						FROM `link_history` 
	   						WHERE `DesiredArrivalTime` between '{$dates['From']}' and '{$dates['To']}' 
	   							and `LinkStatus` in ('COMPLETE','CANCELEDLATE')
	   							and (CustomTransitionType is NULL or CustomTransitionType = 'RIDER')
...
<table class="center_cell_data" border="1" width="870px">
   ...
   <tr>
      ...
      <td><?php echo $link_percentage; ?></td>
   </tr>
...
<table class="center_cell_data" border="1" width="850px" id="ride_collapse">
   ...
   <?php
        if ($dates['From'] && $dates['To']) {
            $date_clause = "DesiredArrivalTime BETWEEN cast('" . mysql_real_escape_string($dates['From']) . 
                                    "' as datetime) AND cast('" . mysql_real_escape_string($dates['To']) . "' as datetime) ";
        } elseif ($dates['From']) {
            $date_clause = "DesiredArrivalTime >= cast('" . mysql_real_escape_string($dates['From']) . "' as datetime) "; 
        } elseif ($dates['To']) {
            $date_clause = "DesiredArrivalTime <= cast('" . mysql_real_escape_string($dates['To']) . "' as datetime) "; 
        } else {
            $date_clause = '1=1 ';
        }
   ...
   <tr>
      ...
      <?php
         $sql = "SELECT *, IF( (SELECT DesiredArrivalTime 
                                      ...
                                      WHERE DriverUserID = t1.DriverUserID ORDER BY DesiredArrivalTime LIMIT 1) BETWEEN 
                                            cast('{$dates['From']}' as datetime) AND cast('{$dates['To']}' as datetime), 'Yes','No') AS New 
...
#echo $sql;
         ...
         while($row = mysql_fetch_array($result)){
            $user_id = get_driver_user_id($row['DriverUserID']);
            $name = get_user_person_name( $user_id );
         echo '<tr>';
            echo "<td><a href=\"account.php\" class=\"User_Redirect\" id=\"$user_id\">$user_id</a></td>";
            echo '<td>' .  get_displayable_person_name_string($name) . "</td>";
