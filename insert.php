<?php
	...
	if(!current_user_has_role($franchise, "Franchisee") && ! current_user_has_role(1, "FullAdmin")){
	...
	if(isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['id'])){
		...
		if(isset($_POST['Text'])){
			...
			if(current_user_has_role(1, "FullAdmin"))
				...
				$safe_franchise = mysql_real_escape_string(get_current_user_franchise());
		...
		<form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>?action=add">
			<table style="margin:auto;">
                ...
                <?php if(current_user_has_role(1, "FullAdmin")){ ?>
				<tr>
					<td>Franchise</td>
					<td>
						<select name="Franchise">
							<?php
								...
								while($row2 = mysql_fetch_array($result)){
									echo '<option value="' . $row2['FranchiseID'] . '"';
									if($row2['FranchiseID'] == $row['FranchiseID'])
										echo ' SELECTED';
									echo '>' . $row2['FranchiseName'] . '</option>';
								}
